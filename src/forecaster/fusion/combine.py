...
def _profile_weights(cfg: Config, profile: str) -> tuple[float, float, float]:
    profiles: dict[str, tuple[float, float, float]] = {
        ...
        # "learned" uses the trained model for the score; these weights are only
        # the fallback when no model / features are available.
        "learned": (cfg.news_weight, cfg.technical_weight, cfg.neutral_band),
    }
...
def combine(
    ...
    *,
    ...
    use_learned = profile == "learned" and learned_score is not None
    if use_learned:
        # The model already weighed every signal (incl. news) from data — take
        # its score directly; confidence is how far it leans from a coin flip.
        final_score = max(-1.0, min(1.0, learned_score))
        final_confidence = abs(final_score)
    else:
        # If the news side produced no usable signal (no Groq key, no articles,
        # or a genuinely no-bearing read → confidence 0), don't let it dilute the
        # blend toward neutral: hand its weight to the technical side so the
        # prediction rests on the information we actually have. Only when the
        # profile still gives technicals a say — a pure "news_only" run with no
        # news stays neutral.
        ...
        # Confidence is a weight-aware blend of each side's own conviction, so a
        # side with no weight in this profile can't drive it. The agreement bonus
        # / disagreement penalty applies only when BOTH sides carry weight AND
        # give a directional (non-neutral) read — a merely-neutral side used to
        # be mistaken for "disagreement" and wrongly halve the confidence.
